...
def motion_segmentation(basedir, threshold):
    ...
    img_list = sorted(list((Path(basedir) / 'images').glob('*')))
    camera_list = sorted(list((Path(basedir) / 'camera').glob('*')))

    with camera_list[0].open('r') as f:
        cam_data = json.load(f)

    hwf = [cam_data['image_size'][1], cam_data['image_size'][0], cam_data['focal_length']]
    ...
    def get_pose(idx):
        with camera_list[idx].open('r') as f:
            cam_data = json.load(f)
        c2w_rot = np.array(cam_data['orientation']).T @ np.diag([1,-1,-1])
        c2w_trans = cam_data['position']
        ...
        # w2c_mat = np.zeros([4,4])
        # w2c_mat[:3,:3] = c2w_rot.T
        # w2c_mat[:3,3] = -1 * c2w_trans
        # w2c_mat[3,3] = 1
        return np.linalg.inv(c2w_mat)
    ...
    for i in range(0, num_frames): #len(perm) - 1):
        ...
        T_prev_G = get_pose(max(0, i - 1))        
        T_ref_G = get_pose(i)
        T_post_G = get_pose(min(num_frames -1, i + 1))
